...
public class JwtServices {
    ...
    public boolean verifyUser(String jwtToken, String user){
        try{
            ...
            Claims tokenData = Jwts.parser().verifyWith(getSignKey()).build().parseSignedClaims(jwtToken).getPayload();
    ...
    public String getUserIdFromJwt(String jwtToken) {
        Claims claimData = Jwts.parser().verifyWith(getSignKey()).build().parseSignedClaims(jwtToken).getPayload();
